...
*
* Combines minified versions of 
* common jquery plugins used throughout kc.com, including:
*
*    jcarousel           -- make images scroll to side, etc.
*    cycle 2.7.3         -- fade, rotate, and otherwise manipulate DOM nodes
     swipe.js            -- mobile swipe events -- https://github.com/bradbirdsall/Swipe
...
* ^ Let MI aggregator know it can minify
*
* Common custom functions used by v3 framework. Includes...
* 	- safe console logging              -- note, this may be covered already by MI JS. Should probably verify.
* 	- image loader                      -- allows deferred loading of images with data-deferred-load attribute
* 	- simple trigger handling           -- simply apply trigger class to link, and it's anchor hash will be toggled 
*														and an "activated" class will be added to the anchor
* 	- simple tabs								-- can use any child elements, and supports remote navigation.
* 	- set_asset_offset						-- Used on story pages to ensure bottom assets clear top assets.
														also includes some additional story-page functions.
														These should probably be wrapped in an "if body.story_detail" check
...
function load_images() {
	$('*[data-deferred-load]').prepend(function(index){
...
/* tabs can be used two ways:
	...
	When tabs are activated, it will be given the class .tabbed, so you can style it appropriately 
	and know you're only styling blocks that have actually been tabbed.
...
function tabit() { 
	$('.tabs').each(function() {
		...
		tabbed.nav.find('a').click(function(e) {
			...
			// IE9 is funny about parent. Use var to keep it out of global scope and IE9 will be happy.
...
var collapsed = false;
function getYOffset() { 
	// helper function for sticky header scroll thing
    var pageY;
...
$(window).scroll(function () {
	if (getYOffset() > 70 && collapsed==false) {
		collapsed = true;
		log(getYOffset());
		$('body').addClass('collapsed');
	}; 
	if (getYOffset() < 40 && collapsed==true) {
		collapsed = false;
		log('uncollapse');
...
// do we have a media block that has set a vmix_genre?
// Build the video list
if (typeof(vmix_genre) != "undefined" | typeof(vmix_collection) != "undefined") {
	vidstring = '<ul>';
	if (typeof(vmix_vid_count) == "undefined") {
		vmix_vid_count = 5;
	}
	if (typeof(vmix_collection) != "undefined") {
		vmix_url = '/cgi-bin/php/vmixer.php?api=media&action=getMediaList&params='+escape('collection_ids='+vmix_collection);
	} else {
		vmix_url = '/cgi-bin/php/vmixer.php?api=media&action=getMediaList&params='+escape('genre_ids='+ vmix_genre)		
	}
	$.getJSON(vmix_url+ escape('&limit='+ vmix_vid_count+'&alltime=1'), function(r) {
		for (i in r){
			if (r[i].title) {
	   		inti = parseInt(i)+1;
	   		vidstring+='<li><a href="/video?token='+ r[i].token +'" rel="'+ r[i].id+'">'+
							 '<img src="http://image.vmixcore.com/imgman.jpg?width=125&url='+r[i].thumbnail[0].url+'" alt="">'+
							 r[i].title.replace('Raw Video: ','')+'</a></li>';
			}
		}
		vidstring += "</ul>";
		$('#media_block .html_module').eq(0).html(vidstring);
	});
}

// Call any functions we need to call
tabit();




if ($('body.story_detail').length > 0) {
	// functions specific to story pages
	// note this is after the tabit() call, so it calculates
	// based on tabbed top asset height, not default.
	
	// sets top margin for more assets on story pages, to ensure it's pushed below top assets
	function set_asset_offset() {
		top_assets = $('.story_detail #top_assets');
		console.log(top_assets.length);
		if (top_assets.length > 0) {
			offset = $('.story_detail #top_assets').height()+20
			$('.story_detail #more_assets').css('margin-top', offset)
		}
	}
	// create preview tooltips in story detail top assets
	$('#top_assets .tabbed ul li a').each(function(e) {
		img = $(this.hash).find('img').eq(0);
		preview = '<span class="preview"><img src="'+ $(img).attr('src') + '"></span>'
		if ($(img).size() == 0) {
			img = $(this.hash).find('object').eq(0);
			preview = '<span class="preview">Video</span>'
		}
		$(this).append(preview)
	})
	$('#top_assets .tabbed ul li').hover(function(e) {
		$(this).find('.preview').toggle()
	})
	$(document).ready(set_asset_offset());
	setTimeout(function(){set_asset_offset() }, 1000);
}
